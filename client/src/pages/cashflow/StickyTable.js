...
export const StickyTable = (props) => {
      ...
      const apiCall = async () => {
        ...
        for(let i=0;i<jsonData.length;i++){
          if(jsonData[i].category==="I"){
            if(inflow.length>=1 && jsonData[i].CashFlow!=="Total Cash" && inflow[inflow.length-1].CashFlow==="Total Cash"){
            ...
            if(outflow.length>=1 && jsonData[i].CashFlow!=="Ending Cash" && jsonData[i].CashFlow!=="Total Expenses" &&
    ...
    //comment 3
      ...
      const handleDelete=(id)=>{
        ...
        fetch(`http://localhost:8000/deleteRow/${countryobj.country}/${data[id].CashFlow}`,{method:"DELETE"}).then(resp=>apiCall());
    ...
    upobj.country=countryobj.country;
        //console.log(data[id].CashFlow);
        fetch(`http://localhost:8000/updateRow/${data[id].CashFlow}`,{method:"PUT",body:JSON.stringify(upobj),
        headers:{
...
category:"I"});
  ...
  const handleFormData=(data)=>{
    ...
    console.log("updating data from the form");
        fetch('http://localhost:8000/addRow/',{method:"POST",body:JSON.stringify(data),
      headers:{
        'content-type':"application/json"
      }}).then(resp=>apiCall());
  }
  ...
  return (
    ...
    <div className='apptable'>
        <table {...getTableProps()} className="table sticky" style={{ width: 1000, height: 500 }}>
    ...
    <Grid align="right">
      <Button variant="contained" color="primary" onClick={handleClickOpen}>{role===true?'Add user':'Print'}</Button> 
    </Grid>
    <FormDialog open={open}  handleClose={handleClose} data={formData} onChange={onChange} handleFormData={handleFormData}/>
