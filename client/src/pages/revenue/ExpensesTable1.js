...
import { COLUMNS } from "../revenue/columns";
import { mock } from "../revenue/expensesdata";
import "../select.css";
...
export const ExpenseTable1 = (props) => {
  const { role } = props;
  console.log(role);
  ...
  const apiCall = async () => {
    ...
    newob.projectname = "Total Expenses";
    let month = [
      "jan",
      "feb",
      "mar",
    ...
    for (let i = 0; i < month.length; i++) {
      ...
      for (let j = 0; j < jsonData.length - 1; j++) {
        s += jsonData[j][month[i]];
      }
      newob[month[i]] = s;
    }
    let newData = new Array(...jsonData);
    newData.splice(newData.length - 1, 0, newob);
  ...
  const handleDelete = (id) => {
    fetch(`http://localhost:8000/revenue/deleteRow/${data[id].projectname}`, {
      method: "DELETE",
    }).then((resp) => apiCall());
  };
  ...
  const handleUpdate = (id) => {
    ...
    fetch(`http://localhost:8000/revenue/updateRow/${data[id].projectname}`, {
      method: "PUT",
  ...
  const handleFormData = (data1) => {
    ...
    fetch("http://localhost:8000/revenue/addRow/", {
      method: "POST",
