...
function Hardware() {
  ...
  const apiCall = async () => {
    ...
    const response = await fetch(`http://localhost:8000/capital/hardware`);
  ...
  const onDeleteStudent = (record) => {
    Modal.confirm({
      ...
      okType: "danger",
      onOk: () => {
        fetch(`http://localhost:8000/capital/hardware/deleteRow/${record.id}`, {
          method: "DELETE",
        }).then(() => {
          setDataSource((pre) => {
      ...
      <header className="App-header">
            ...
            setDataSource((pre) => {
              return pre.map((student) => {
                if (student.id === editingStudent.id) {
                  ...
                  fetch(
                    `http://localhost:8000/capital/hardware/updateRow/${student.id}`,
                    {
                      method: "PUT",
        ...
        <Modal
          ...
          onOk={() => {
            ...
            console.log(ob);
            fetch("http://localhost:8000/capital/hardware/addRow/", {
              method: "POST",
              body: JSON.stringify(ob),
              headers: {
                "content-type": "application/json",
